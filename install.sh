...
if [ "$(uname -m)" = "armv7l" ]; then
  echo ""
  echo "WARNING:"
  echo "The LotteryCoin requires a 64 bit OS and this is 32 bit armv7l"
...
if [ "$PYTHON_MAJOR_VER" -ne "3" ] || [ "$PYTHON_MINOR_VER" -lt "7" ] || [ "$PYTHON_MINOR_VER" -ge "13" ]; then
  echo "LotteryCoin requires Python version >= 3.8 and  < 3.13.0" >&2
...
echo ""
echo "LotteryCoin install.sh complete."
echo "For assistance join us on Discord in the #support chat channel:"
echo "https://discord.com/invite/pTMp9R4v2w"
echo ""
echo "To install the GUI run '. ./activate' then 'sh install-gui.sh'."
echo ""
echo "Type '. ./activate' and then 'lottery init' to begin."