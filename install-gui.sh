...
if [ "$(id -u)" = 0 ]; then
  echo "The LotteryCoin GUI can not be installed or run by the root user."
  exit 1
fi

# Allows overriding the branch or commit to build in LotteryCoin-gui
SUBMODULE_BRANCH=$1
...
if [ ! "$CI" ]; then
  ...
  # Set modified output of `lottery version` to version property of GUI's package.json
...
echo ""
echo "LotteryCoin install-gui.sh completed."
echo ""
