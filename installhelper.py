...
# Gets the version from the result of "lottery version"
...
def get_lottery_version() -> str:
    version: str = "0.0"
    lottery_executable = shutil.which("lottery")
    if lottery_executable is None:
        lottery_executable = "lottery"
    output = subprocess.run([lottery_executable, "version"], capture_output=True)
...
def update_version(package_json_path: str):
    ...
    data["version"] = get_lottery_version()
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/LotteryCoin-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/LotteryCoin-gui/packages/gui/package.json")
