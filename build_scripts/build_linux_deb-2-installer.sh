...
export LOTTERY_INSTALLER_VERSION
...
pip install j2cli
CLI_DEB_BASE="LotteryCoin-cli_$LOTTERY_INSTALLER_VERSION-1_$PLATFORM"
mkdir -p "dist/$CLI_DEB_BASE/opt/lottery"
mkdir -p "dist/$CLI_DEB_BASE/usr/bin"
mkdir -p "dist/$CLI_DEB_BASE/DEBIAN"
mkdir -p "dist/$CLI_DEB_BASE/etc/systemd/system"
LOTTERY_DEB_CONTROL_VERSION=$(format_deb_version_string "$LOTTERY_INSTALLER_VERSION")
export LOTTERY_DEB_CONTROL_VERSION
j2 -o "dist/$CLI_DEB_BASE/DEBIAN/control" assets/deb/control.j2
cp assets/systemd/*.service "dist/$CLI_DEB_BASE/etc/systemd/system/"
cp -r dist/daemon/* "dist/$CLI_DEB_BASE/opt/lottery/"

ln -s ../../opt/lottery/lottery "dist/$CLI_DEB_BASE/usr/bin/lottery"
...
if [ "$PLATFORM" = "arm64" ]; then
  ...
  echo USE_SYSTEM_FPM=true npx electron-builder build --linux deb --arm64 \
    ...
    --config.deb.packageName="LotteryCoin" \
...
GUI_DEB_NAME=LotteryCoin_${LOTTERY_INSTALLER_VERSION}_${PLATFORM}.deb
mv "dist/${PRODUCT_NAME}-${LOTTERY_INSTALLER_VERSION}.deb" "../../../build_scripts/dist/${GUI_DEB_NAME}"
