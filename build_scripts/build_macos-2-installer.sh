...
"${NPM_PATH}/electron-builder" build --mac "${OPT_ARCH}" \
  ...
  --config ../../../build_scripts/electron-builder.json
LAST_EXIT_CODE=$?
ls -l dist/mac*/lottery.app/Contents/Resources/app.asar
...
ORIGINAL_DMG_NAME="lottery-${LOTTERY_INSTALLER_VERSION}.dmg"
if [ "$(arch)" = "arm64" ]; then
  DMG_NAME=LotteryCoin-${LOTTERY_INSTALLER_VERSION}-arm64.dmg
else
  # NOTE: when coded, this changes the case to Lottery
  DMG_NAME=LotteryCoin-${LOTTERY_INSTALLER_VERSION}.dmg
fi
...
# xcrun notarytool submit --wait --apple-id username --password password --team-id team-id LotteryCoin-0.1.X.dmg
# Wait until the command returns a success message
#
# Once that is successful, execute the following command":
# xcrun stapler staple LotteryCoin-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate LotteryCoin-0.1.X.dmg
