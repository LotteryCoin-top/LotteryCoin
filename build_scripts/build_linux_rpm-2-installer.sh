...
if [ ! "$LOTTERY_INSTALLER_VERSION" ]; then
  echo "WARNING: No environment variable LOTTERY_INSTALLER_VERSION set. Using 0.0.0."
  LOTTERY_INSTALLER_VERSION="0.0.0"
fi
echo "Lottery Installer Version is: $LOTTERY_INSTALLER_VERSION"
...
# Builds CLI only rpm
CLI_RPM_BASE="LotteryCoin-cli-$LOTTERY_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/lottery"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
mkdir -p "dist/$CLI_RPM_BASE/etc/systemd/system"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/lottery/"
cp assets/systemd/*.service "dist/$CLI_RPM_BASE/etc/systemd/system/"

ln -s ../../opt/lottery/lottery "dist/$CLI_RPM_BASE/usr/bin/lottery"
...
fpm -s dir -t rpm \
  ...
  --name LotteryCoin-cli \
  --license Apache-2.0 \
  --version "$LOTTERY_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "Lottery is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
  ...
  .
# CLI only rpm done
cp -r dist/daemon ../LotteryCoin-gui/packages/gui
# Change to the gui package
cd ../LotteryCoin-gui/packages/gui || exit 1

# sets the version for LotteryCoin in package.json
cp package.json package.json.orig
jq --arg VER "$LOTTERY_INSTALLER_VERSION" '.version=$VER' package.json >temp.json && mv temp.json package.json
...
PRODUCT_NAME="lotteryCoin"
echo "${NPM_PATH}/electron-builder" build --linux rpm "${OPT_ARCH}" \
  --config.extraMetadata.name=LotteryCoin \
  --config.productName="${PRODUCT_NAME}" --config.linux.desktop.Name="LotteryCoin" \
  --config.rpm.packageName="LotteryCoin" \
...
GUI_RPM_NAME="LotteryCoin-${LOTTERY_INSTALLER_VERSION}-1.${REDHAT_PLATFORM}.rpm"
mv "dist/${PRODUCT_NAME}-${LOTTERY_INSTALLER_VERSION}.rpm" "../../../build_scripts/dist/${GUI_RPM_NAME}"
