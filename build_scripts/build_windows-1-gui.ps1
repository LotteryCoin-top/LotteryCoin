...
git submodule update --init LotteryCoin-gui

Set-Location -Path ".\LotteryCoin-gui" -PassThru
...
# Other than `LotteryCoin-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @lottery-network packages to make cache slim"
...
# Remove `packages/gui/node_modules/@lottery-network` because it causes an error on later `electron-packager` command
#Remove-Item "@lottery-network" -Recurse -Force
