...
from chia.cmds.init_funcs import check_keys, lottery_init
...
from chia.util.lottery_logging import initialize_service_logging
from chia.util.lottery_version import lottery_short_version
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Lottery.")
    sys.exit()
...
if getattr(sys, "frozen", False):
    name_map = {
        "lottery_full_node_simulator": "lottery_simulator",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Lottery will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["lottery", "plotters", plotter]

        if plotter == "bladebit":
            # plotter command must be either
            # 'lottery plotters bladebit ramplot' or 'lottery plotters bladebit diskplot'
            plot_type = request["plot_type"]
    ...
    async def exit(self) -> None:
        ...
        log.info("lottery daemon exiting")
...
def launch_plotter(
    ...
    # we need to pass on the possibly altered LOTTERY_ROOT
    os.environ["LOTTERY_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with LOTTERY_ROOT: {os.environ['LOTTERY_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in lottery_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    lottery_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("lottery_daemon")
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"LotteryCoin version: {lottery_short_version()}")
