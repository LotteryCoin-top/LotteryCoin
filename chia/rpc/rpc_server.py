...
from chia.server.server import LotteryServer, ssl_context_for_client, ssl_context_for_server
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> LotteryServer:
        ...
        # a property so as to be read only which allows LotteryServer to satisfy
        # Optional[LotteryServer]
        ...
...
def default_get_connections(server: LotteryServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
