...
class WalletRpcClient(RpcClient):
    """
    Client to Lottery RPC, connects to a local wallet. Uses HTTP/JSON, and converts back from
    JSON into native python objects before returning. All api calls use POST requests.
    Note that this is not the same as the peer protocol, or wallet protocol (which run Lottery's
    ...
    async def nft_mint_bulk(
        ...
        lot_coins: Optional[List[Dict[str, Any]]] = None,
        lot_change_target: Optional[str] = None,
        ...
        request = {
            ...
            "lot_coins": lot_coins,
            "lot_change_target": lot_change_target,
    ...
    async def recover_pool_nft(
        self, launcher_id: str, contract_address: str, fee: uint64 = uint64(0)
    ) -> RecoverPoolNFTResponse:
        ...
        response = await self.fetch("recover_pool_nft", request)
        return json_deserialize_with_clvm_streamable(response, RecoverPoolNFTResponse)
    ...
    async def send_stake(
        ...
        response = await self.fetch("send_stake", request)
        return json_deserialize_with_clvm_streamable(response, SendStakeResponse)
    ...
    async def spend_withdraw_coins(
        ...
        response = await self.fetch("spend_withdraw_coins", request)
        return response

    async def guess_send(
        self,
        wallet_id: int,
        amount: uint64,
        tx_config: TXConfig,
        ...
        response = await self.fetch("send_guess", request)
        return json_deserialize_with_clvm_streamable(response, SendGuessResponse)
