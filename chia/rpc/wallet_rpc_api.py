...
from chia.consensus.block_rewards import calculate_base_farmer_reward, MOJO_PER_LOTTERY
...
from chia.pools.pool_puzzles import POOL_OUTER_MOD_HASH, create_p2_singleton_puzzle
...
from chia.types.stake_record import get_stake_value, STAKE_LOCK_MIN_MOJO, STAKE_LOCK_LIST
from chia.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from chia.util.byte_types import hexstr_to_bytes
from chia.util.config import load_config, str2bool
from chia.util.errors import KeychainIsLocked
from chia.util.guess import check_guess_memos, GUESS_PUZZLE_HASH
...
from chia.wallet.conditions import (
    ...
    CreatePuzzleAnnouncement, ConditionValidTimes,
)
...
from chia.wallet.puzzles.stake.metadata import StakeMetadata, AutoWithdrawStakeSettings
...
class WalletRpcApi:
    ...
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "lottery_wallet"
    ...
    def get_routes(self) -> Dict[str, Endpoint]:
        return {
            ...
            "/send_stake": self.send_stake,
            "/spend_withdraw_coins": self.spend_withdraw_coins,
            # guess
            "/send_guess": self.send_guess,
        }
    ...
    async def get_latest_singleton_coin_spend(
            self, peer: WSLotteryConnection, coin_id: bytes32, latest: bool = True
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("lot_target_address", "")
        pool_target = config["pool"].get("lot_target_address", "")
    ...
    async def get_transactions(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        for tr in transactions:
            ...
            try:
                ...
                if tx["type"] == TransactionType.INCOMING_STAKE_LOCK_RECEIVE.value:
    ...
    async def verify_signature(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        # signatures made by `lottery keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
        ...
        if signing_mode == SigningMode.CHIP_0002 or signing_mode == SigningMode.CHIP_0002_P2_DELEGATED_CONDITIONS:
            ...
            #   ("Lottery Signed Message", message)
    ...
    async def cancel_offers(
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "lot")
        ...
        key = None
        if asset_id is not None and asset_id != "lot":
    ...
    async def did_get_info(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.LOT.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(
        ...
        lot_coin_list = request.get("lot_coins", None)
        lot_coins = None
        if lot_coin_list:
            lot_coins = {Coin.from_json_dict(lot_coin) for lot_coin in lot_coin_list}
        lot_change_target = request.get("lot_change_target", None)
        if lot_change_target is not None:
            if lot_change_target[:2] == "lot":
                lot_change_ph = decode_puzzle_hash(lot_change_target)
            else:
                lot_change_ph = bytes32(hexstr_to_bytes(lot_change_target))
        else:
            lot_change_ph = None
        ...
        if mint_from_did:
            ...
            await nft_wallet.mint_from_lot(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
    ...
    async def pw_join_pool(
        ...
        async with self.service.wallet_state_manager.lock:
            total_fee = await wallet.join_pool(new_target_state, fee, tx_config, action_scope)
            return {
                "total_fee": total_fee,
                "transaction": None,  # tx_endpoint wrapper will take care of this
    ...
    async def pw_self_pool(
        ...
        async with self.service.wallet_state_manager.lock:
            total_fee = await wallet.self_pool(fee, tx_config, action_scope)
            return {
                "total_fee": total_fee,
                "transaction": None,  # tx_endpoint wrapper will take care of this
                "fee_transaction": None,  # tx_endpoint wrapper will take care of this
    ...
    async def pw_absorb_rewards(
        ...
        assert isinstance(wallet, PoolWallet)
        async with self.service.wallet_state_manager.lock:
            await wallet.claim_pool_rewards(fee, max_spends_in_tx, tx_config, action_scope)
    ...
    ##########################################################################################
    # Wallet Recover Pool NFT Management
    ##########################################################################################
    ...
    async def recover_pool_nft(
        ...
        try:
            ...
            if len(coin_spends) > 0:
                ...
                if fee > 0:
                    ...
                    async with self.service.wallet_state_manager.lock:
                        [fee_tx] = await wallet.generate_signed_transaction(
                            uint64(0),
                            (await wallet.get_puzzle_hash(False)),
                            tx_config,
                            action_scope,
                            fee=fee,
                            origin_id=None,
                ...
                target_puzzle_hash = self.pool_nft_puzzle_hash.get(launcher_id)
    ...
    async def set_auto_withdraw_stake(self, request: Dict[str, Any]) -> EndpointResult:
        """
        Set auto claim merkle coins config
        :param request: Example {"enable": true, "tx_fee": 100000, "batch_size": 50}
        :return:
        """
        return self.service.set_auto_withdraw_stake(AutoWithdrawStakeSettings.from_json_dict(request))

    async def get_auto_withdraw_stake(self, request: Dict[str, Any]) -> EndpointResult:
        """
        Get auto claim merkle coins config
        :param request: None
        :return:
    ...
    async def send_stake(
        ...
        if get_stake_value(stake_type).time_lock == 0:
            raise ValueError(f"stake type error {stake_type}")

        fee: uint64 = uint64(request.get("fee", 0))
        amount: uint64 = uint64(request["amount"])
        if amount < STAKE_LOCK_MIN_MOJO:
            raise ValueError(f"stake lock must more then {int(STAKE_LOCK_MIN_MOJO / MOJO_PER_LOTTERY)} LOT")
        ...
        if address != "":
            ...
            expected_prefix = self.service.config["network_overrides"]["config"][selected_network]["address_prefix"]
            if address[0: len(expected_prefix)] != expected_prefix:
                raise ValueError(f"Unexpected stake lock address Prefix: {expected_prefix}")
            puzzle_hash: bytes32 = decode_puzzle_hash(address)
        else:
            puzzle_hash: bytes32 = await wallet.get_puzzle_hash(False)
    ...
    async def spend_withdraw_coins(
        ...
        for coin_id, coin_record in coin_records.coin_id_to_record.items():
            try:
                ...
                log.error(f"Failed to spend withdraw coin {coin_id.hex()}: %s", e)
        if len(coins) > 0:
            await self.service.wallet_state_manager.spend_stake_coins(
                coins,
                tx_fee,
    ...
    async def send_guess(
        ...
        amount: uint64 = uint64(request["amount"])
        fee: uint64 = uint64(request.get("fee", 0))
        if fee < int(0.01 * MOJO_PER_LOTTERY):
            raise ValueError(f"stake fee must >= 0.01")

        if "data" not in request:
            raise ValueError("Guess data is empty")
        guess_err, guess_data = check_guess_memos(amount, request["data"])
        if guess_err is not None:
            raise ValueError(guess_err)

        selected_network = self.service.config["selected_network"]
        expected_prefix = self.service.config["network_overrides"]["config"][selected_network]["address_prefix"]

        if "award_address" in request and request["award_address"] != "":
            address = request["award_address"]
            if address[0: len(expected_prefix)] != expected_prefix:
                raise ValueError("Unexpected Award Address Prefix")
            puzzle_hash = decode_puzzle_hash(address)
        else:
            puzzle_hash = await wallet.get_puzzle_hash(False)
        guess_data["W"] = puzzle_hash.hex()

        if "rebate" in request:
            guess_data["R"] = str(request["rebate"])
        ...
        async with self.service.wallet_state_manager.lock:
            await wallet.generate_signed_transaction(
                ...
                action_scope,
                fee,
                memos=[json.dumps(guess_data).encode("utf-8")],
                extra_conditions=extra_conditions,
            )
