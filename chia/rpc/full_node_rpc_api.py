...
class FullNodeRpcApi:
    def __init__(self, service: FullNode) -> None:
        ...
        self.service_name = "lottery_full_node"
    ...
    async def get_blockchain_state(self, _: Dict[str, Any]) -> EndpointResult:
        ...
        if peak is not None and peak.height > 1:
            ...
            average_block_time = await get_average_block_time(
                self.service.blockchain, transaction_block, 4608
            )
            stake_lock, stake_lock_calc = (
                await self.service.blockchain.stake_store.get_stake_amount_total(transaction_block.timestamp)
            )
        else:
            stake_lock: int = 0
    ...
    def _get_spendbundle_type_cost(self, name: str) -> uint64:
        ...
        tx_cost_estimates = {
            "send_lot_transaction": 9_401_710,
    ...
    async def get_stake_records(self, request: Dict) -> EndpointResult:
        ...
        if peak_height is None or header_height > peak_height:
            raise ValueError(f"Block height {height} not found in chain")
        records: List[StakeRecord] = await self.service.blockchain.stake_store.get_stake_records(header_height)
