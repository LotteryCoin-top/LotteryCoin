...
from chia.full_node.stake_store import StakeStore
...
class FullNode:
    ...
    _stake_store: Optional[StakeStore] = None
    ...
    async def manage(self) -> AsyncIterator[None]:
        ...
        async with DBWrapper2.managed(
                    ...
                    async with conn.execute(
                        ...
                        if len(list(await cur.fetchall())) == 0:
                            try:
                                ...
                                # it could be a database created with "lottery init", which is
            ...
            self._stake_store = await StakeStore.create(self.db_wrapper)
    ...
    @property
    def stake_store(self) -> StakeStore:
        assert self._stake_store is not None
        return self._stake_store
    ...
    def set_server(self, server: LotteryServer) -> None:
        ...
        if "dns_servers" in self.config:
            ...
            dns_servers.append("dns-introducer.lotterycoin.top")
        try:
    ...
    async def short_sync_batch(self, peer: WSLotteryConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: WSLotteryConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ) -> bool:
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: WSLotteryConnection) -> None:
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[WSLotteryConnection] = None
    ) -> None:
        """
    ...
    async def on_connect(self, connection: WSLotteryConnection) -> None:
        """
    ...
    async def on_disconnect(self, connection: WSLotteryConnection) -> None:
    ...
    async def request_validate_wp(
        ...
        weight_proof_peer: WSLotteryConnection = random.choice(peers_with_peak)
        self.log.info(
    ...
    async def sync_from_fork_point(
        ...
        peers_with_peak: List[WSLotteryConnection] = self.get_peers_with_peak(peak_hash)
        ...
        async def fetch_block_batches(
            batch_queue: asyncio.Queue[Optional[Tuple[WSLotteryConnection, List[FullBlock]]]]
        ) -> None:
            start_height, end_height = 0, 0
            new_peers_with_peak: List[WSLotteryConnection] = peers_with_peak[:]
            try:
        ...
        async def validate_block_batches(
            inner_batch_queue: asyncio.Queue[Optional[Tuple[WSLotteryConnection, List[FullBlock]]]]
        ) -> None:
            fork_info: Optional[ForkInfo] = None

            while True:
                res: Optional[Tuple[WSLotteryConnection, List[FullBlock]]] = await inner_batch_queue.get()
                if res is None:
        ...
        batch_queue_input: asyncio.Queue[Optional[Tuple[WSLotteryConnection, List[FullBlock]]]] = asyncio.Queue(
            maxsize=buffer_size
        )
    ...
    def get_peers_with_peak(self, peak_hash: bytes32) -> List[WSLotteryConnection]:
    ...
    async def peak_post_processing(
        ...
        peer: Optional[WSLotteryConnection],
    ...
    async def add_unfinished_block(
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'lot')}, "
                f"validation time: {validation_time:0.4f} seconds, {pre_validation_log}"
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
        ...
        def old_clients(conn: WSLotteryConnection) -> bool:
            # don't send this to peers with new clients
            return conn.protocol_version <= Version("0.0.35")

        def new_clients(conn: WSLotteryConnection) -> bool:
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[WSLotteryConnection] = None
    ) -> Optional[Message]:
    ...
    async def add_end_of_sub_slot(
        self, end_of_slot_bundle: EndOfSubSlotBundle, peer: WSLotteryConnection
    ...
    async def broadcast_added_tx(
        self, mempool_item: MempoolItem, current_peer: Optional[WSLotteryConnection] = None
    ) -> None:
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: WSLotteryConnection) -> None:
        peak = self.blockchain.get_peak()
        if peak is None or peak.height - request.height < 5:
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: WSLotteryConnection) -> None:
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def add_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: WSLotteryConnection) -> None:
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
...
async def node_next_block_check(
    peer: WSLotteryConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
    block_response: Optional[Any] = await peer.call_api(
