...
from chia.plotters.chiapos import get_chiapos_install_info, plot_lottery
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in LOTTERY_ROOT.
    lottery_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within LOTTERY_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
            print(f"Cannot create plotters root path {root_path} {type(e)} {e}.")

    plotters = argparse.ArgumentParser("lottery plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, chia_plotter_options, "chiapos", "Create a plot with the default lottery plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'lottery plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    subparsers.add_parser("install", help=deprecation_warning, add_help=False)

    deprecation_warning_bb2 = "[DEPRECATED] 'lottery plotters bladebit2' was integrated to 'lottery plotters bladebit'"
    ...
    if args.plotter is None:
        ...
        plot_lottery(args, lottery_root_path)
    elif args.plotter == "madmax":
        plot_madmax(args, lottery_root_path, root_path)
    elif args.plotter == "bladebit":
        plot_bladebit(args, lottery_root_path, root_path)
    elif args.plotter == "version":
        show_plotters_version(lottery_root_path)
