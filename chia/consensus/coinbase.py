...
def lottery_bonus_parent_id(block_height: uint32, genesis_challenge: bytes32) -> bytes32:
    return bytes32(genesis_challenge[8:24] + block_height.to_bytes(16, "big"))


def stake_reward_parent_id(block_height: uint32, genesis_challenge: bytes32) -> bytes32:
    return bytes32(genesis_challenge[12:28] + block_height.to_bytes(16, "big"))


def create_lottery_bonus_coin(block_height: uint32, puzzle_hash: bytes32, reward: uint64, genesis_challenge: bytes32) -> Coin:
    parent_id = lottery_bonus_parent_id(block_height, genesis_challenge)
    return Coin(parent_id, puzzle_hash, reward)


def create_stake_reward_coin(block_height: uint32, puzzle_hash: bytes32, reward: uint64, genesis_challenge: bytes32) -> Coin:
    parent_id = stake_reward_parent_id(block_height, genesis_challenge)
    return Coin(parent_id, puzzle_hash, reward)
