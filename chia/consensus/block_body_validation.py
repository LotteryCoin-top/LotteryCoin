...
from chia.consensus.block_rewards import (
    ...
    calculate_lottery_bonus,
)
from chia.consensus.block_root_validation import validate_block_merkle_roots
from chia.consensus.blockchain_interface import BlockchainInterface
from chia.consensus.coinbase import create_farmer_coin, create_pool_coin, create_lottery_bonus_coin
from chia.consensus.constants import ConsensusConstants
from chia.consensus.cost_calculator import NPCResult
from chia.consensus.default_constants import FEE_DESTROYED_PUZZLE_HASH, FUNDS_PUZZLE_HASH
...
from chia.types.stake_record import create_stake_lock_rewards
...
class ForkInfo:
    ...
    # the peak_height block_generator
    block_generator_fork: Dict[uint32, Optional[BlockGenerator]] = field(default_factory=dict)
    ...
    def include_spends(
        ...
        self.peak_hash = header_hash
        self.block_generator_fork[height] = block_generator
...
async def validate_block_body(
    ...
    *,
    ...
    if height > 0:
        ...
        if prev_transaction_block_height % ISSUE_HEIGHT_PER_LOTTERY == 0:
            log.info(f"Adding lottery bonus prev_transaction_block_height {prev_transaction_block_height} {calculate_lottery_bonus(prev_transaction_block_height)}")
            lottery_bonus_coin = create_lottery_bonus_coin(
                prev_transaction_block_height,
                FUNDS_PUZZLE_HASH,
                calculate_lottery_bonus(prev_transaction_block_height),
                constants.GENESIS_CHALLENGE,
            )
            expected_reward_coins.add(lottery_bonus_coin)
        if prev_transaction_block.fees > 0:
            fee_coin = create_farmer_coin(
                prev_transaction_block_height,
                FEE_DESTROYED_PUZZLE_HASH,
                prev_transaction_block.fees,
                constants.GENESIS_CHALLENGE,
            )
            expected_reward_coins.add(fee_coin)
        ...
        if prev_transaction_block.height > 0:
            ...
            while not curr_b.is_transaction_block:
                ...
                if curr_b.height % ISSUE_HEIGHT_PER_LOTTERY == 0:
                    log.info(f"Adding lottery bonus coin at height {curr_b.height} {calculate_lottery_bonus(curr_b.height)}")
                    expected_reward_coins.add(
                        create_lottery_bonus_coin(
                            curr_b.height,
                            FUNDS_PUZZLE_HASH,
                            calculate_lottery_bonus(curr_b.height),
                            constants.GENESIS_CHALLENGE,
                        )
                    )
