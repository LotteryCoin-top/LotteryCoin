...
from chia.server.lottery_policy import set_lottery_policy
from chia.server.outbound_message import NodeType
from chia.server.server import LotteryServer
from chia.server.signal_handlers import SignalHandlers
from chia.server.ssl_context import lottery_ssl_ca_paths, private_ssl_ca_paths
...
from ..util.lottery_version import lottery_short_version
...
class Service(Generic[_T_RpcServiceProtocol, _T_ApiProtocol, _T_RpcApiProtocol]):
    def __init__(
        ...
        *,
        ...
        on_connect_callback: Optional[Callable[[WSLotteryConnection], Awaitable[None]]] = None,
        ...
        self._log.info(f"LotteryCoin version: {lottery_short_version()}")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        lottery_ca_crt, lottery_ca_key = lottery_ssl_ca_paths(root_path, self.config)
        ...
        self._server = LotteryServer.create(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def manage(self, *, start: bool = True) -> AsyncIterator[None]:
        ...
        async with contextlib.AsyncExitStack() as async_exit_stack:
            try:
                ...
                self._log.info("Waiting for LotteryServer to be closed")
    ...
    async def setup_process_global_state(self, signal_handlers: SignalHandlers) -> None:
        ...
        proctitle_name = f"lottery_{self._service_name}"
...
def async_run(coro: Coroutine[object, object, T], connection_limit: Optional[int] = None) -> T:
    with log_exceptions(log=log, message="fatal uncaught exception"):
        if connection_limit is not None:
            set_lottery_policy(connection_limit)
