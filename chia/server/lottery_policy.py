...
if TYPE_CHECKING:
    ...
    class BaseEventsServer(asyncio.base_events.Server):
        if sys.platform == "win32":
            _loop: LotteryProactorEventLoop
        else:
...
class PausableServer(BaseEventsServer):
    ...
    def _detach(self) -> None:
        ...
        if (
            ...
            self._lottery_resume()

    def _lottery_pause(self) -> None:
...
async def _lottery_create_server(
    cls: Any,
    protocol_factory: _ProtocolFactory,
    host: Any,
    port: Any,
...
class LotterySelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _lottery_create_server(super(), *args, **kwargs)


if sys.platform == "win32":

    class LotteryProactor(IocpProactor):
        ...
        async def _lottery_accept_loop(self, listener: socket.socket) -> Tuple[socket.socket, Tuple[object, ...]]:
            while True:
                ...
                try:
                    return await self._lottery_accept(listener)
        ...
        def _lottery_accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            ...
            async def accept_coro(self: LotteryProactor, future: asyncio.Future[object], conn: socket.socket) -> None:
        ...
        def accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            coro = self._lottery_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class LotteryProactorEventLoop(ProactorEventLoop):
...
class LotteryPolicy(asyncio.DefaultEventLoopPolicy):
    def new_event_loop(self) -> asyncio.AbstractEventLoop:
        # overriding https://github.com/python/cpython/blob/v3.11.0/Lib/asyncio/events.py#L689-L695
        if sys.platform == "win32":
            loop_factory = LotteryProactorEventLoop
        else:
            loop_factory = LotterySelectorEventLoop

        return loop_factory()


def set_lottery_policy(connection_limit: int) -> None:
    ...
    asyncio.set_event_loop_policy(LotteryPolicy())
