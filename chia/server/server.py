...
from chia.server.ws_connection import ConnectionCallback, WSLotteryConnection
...
class LotteryServer:
    ...
    all_connections: Dict[bytes32, WSLotteryConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        lottery_ca_crt_key: Tuple[Path, Path],
        name: str = __name__,
    ) -> LotteryServer:
        ...
        lottery_ca_crt_path, lottery_ca_key_path = lottery_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                ca_cert=lottery_ca_crt_path,
                ca_key=lottery_ca_key_path,
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSLotteryConnection] = []
    ...
    async def start(
        ...
        if self.webserver is not None:
            raise RuntimeError("LotteryServer already started")
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSLotteryConnection] = None
        try:
            connection = WSLotteryConnection.create(
    ...
    async def connection_added(
        self, connection: WSLotteryConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    async def connection_closed(
        self, connection: WSLotteryConnection, ban_time: int, closed_connection: bool = False
    ) -> None:
    ...
    async def send_to_all_if(
        ...
        messages: List[Message],
        node_type: NodeType,
        predicate: Callable[[WSLotteryConnection], bool],
        exclude: Optional[bytes32] = None,
    ) -> None:
        await self.validate_broadcast_message_type(messages, node_type)
        for _, connection in self.all_connections.items():
    ...
    def is_trusted_peer(self, peer: WSLotteryConnection, trusted_peers: Dict[str, Any]) -> bool:
