...
from chia.util.config import create_default_lottery_config, load_config, save_config
...
def create_config(
    ...
    # create lottery directories
    create_default_lottery_config(lottery_root)
    create_all_ssl(
        lottery_root,
    ...
    # load config
    config = load_config(lottery_root, "config.yaml")
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(keychain, fingerprint), "tlot")
    config["simulator"]["plot_directory"] = "test-simulator/plots"
    # save config
    save_config(lottery_root, "config.yaml", config)
    return config


async def start_simulator(lottery_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    started_simulator = await start_simulator_main(True, automated_testing, root_path=lottery_root)
...
async def get_full_lottery_simulator(
    lottery_root: Path,
    ...
    """
    A lottery root Path is required.
    The lottery root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    This test is fully interdependent and can be used without the rest of the lottery test suite.
    ...
    with Lockfile.create(daemon_launch_lock_path(lottery_root)):
        ...
        crt_path = lottery_root / config["daemon_ssl"]["private_crt"]
        key_path = lottery_root / config["daemon_ssl"]["private_key"]
        ca_crt_path = lottery_root / config["private_ssl_ca"]["crt"]
        ca_key_path = lottery_root / config["private_ssl_ca"]["key"]

        ws_server = WebSocketServer(lottery_root, ca_crt_path, ca_key_path, crt_path, key_path)
        async with SignalHandlers.manage() as signal_handlers:
            await ws_server.setup_process_global_state(signal_handlers=signal_handlers)
            async with ws_server.run():
                async for simulator in start_simulator(lottery_root, automated_testing):
                    yield simulator, lottery_root, config, mnemonic, fingerprint, keychain
