...
# the output will be printed to your lottery root path, e.g. ~/.lottery/mainnet/profile/
# to analyze the profile, run:

#   python chia/utils/profiler.py ~/.lottery/mainnet/profile | less -r

# this will print CPU usage of the lottery full node main thread at 1 second increments.
# find a time window of interest and analyze the profile file (which are in pstats format).

# for example:

#   python chia/utils/profiler.py ~/.lottery/mainnet/profile 10 20
...
if __name__ == "__main__":
    ...
    def analyze_slot_range(profile_dir: pathlib.Path, first: int, last: int) -> None:
        ...
        output_file = f"lottery-hotspot-{first}"
        if first < last:
            output_file += f"-{last}"
