...
def create_default_lottery_config(root_path: Path, filenames: List[str] = ["config.yaml"]) -> None:
...
def _load_config_maybe_locked(
    ...
    if not path.is_file():
        ...
        print("** please run `lottery init` to migrate or create new config files **")
        # TODO: fix this hack
        sys.exit(-1)
