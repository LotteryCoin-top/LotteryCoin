...
class CRCATWallet(CATWallet):
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSLotteryConnection, coin_data: Optional[CATCoinData]
    ) -> None:
        ...
        for coin in coin_records:
            ...
            if first:
                ...
                if need_lottery_transaction:
                    ...
                    elif regular_lottery_to_claim > fee:
    ...
    async def claim_pending_approval_balance(
        ...
        nonce: bytes32 = Program.to(
            [coin_as_list(c) for c in sorted(coins.union(lottery_coins).union({vc.coin}), key=Coin.name)]
        ).get_tree_hash()
