...
async def request_and_validate_removals(
    peer: WSLotteryConnection, height: uint32, header_hash: bytes32, coin_name: bytes32, removals_root: bytes32
) -> bool:
...
async def request_header_blocks(
    peer: WSLotteryConnection, start_height: uint32, end_height: uint32
...
async def _fetch_header_blocks_inner(
    all_peers: List[Tuple[WSLotteryConnection, bool]],
...
async def fetch_coin_spend(height: uint32, coin: Coin, peer: WSLotteryConnection) -> CoinSpend:
...
async def fetch_coin_spend_for_coin_state(coin_state: CoinState, peer: WSLotteryConnection) -> CoinSpend:
