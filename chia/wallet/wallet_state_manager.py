...
from chia.server.server import LotteryServer
from chia.server.ws_connection import WSLotteryConnection
...
from chia.types.stake_record import STAKE_LOCK_MIN_MOJO
from chia.util.bech32m import encode_puzzle_hash
from chia.util.condition_tools import conditions_dict_for_solution, pkm_pairs_for_conditions_dict
from chia.util.db_synchronous import db_synchronous_on
from chia.util.db_wrapper import DBWrapper2
from chia.util.errors import Err
from chia.util.guess import (
    ISSUE_HEIGHT_PER_LOTTERY,
    GUESS_PUZZLE_HASH,
    check_guess_memos,
    generate_bets,
    get_guess_height,
...
from chia.wallet.puzzles.stake.drivers import generate_stake_spend_bundle, match_stake_puzzle
...
from chia.wallet.transaction_record import TransactionRecord, TransactionRecordOld
...
class WalletStateManager:
    ...
    spend_coin_cache: Dict[bytes32, int] = {}
    ...
    async def determine_coin_type(
            self, peer: WSLotteryConnection, coin_state: CoinState, fork_height: Optional[uint32]
        ...
        if coin_state.coin.amount >= STAKE_LOCK_MIN_MOJO:
    ...
    async def spend_clawback_coins(
        ...
        for coin, metadata in clawback_coins.items():
            ...
            try:
                ...
                spend_count = self.spend_coin_cache.get(coin_name, 0) + 1
                if incoming_tx.sent > 0 and not force:
                    self.log.error(
                        f"Clawback coin {coin.name().hex()} is already in a pending spend bundle. {incoming_tx}"
                    )
                    if spend_count < 3:
                        self.spend_coin_cache[coin_name] = spend_count
                        continue
                    else:
                        self.spend_coin_cache.pop(coin_name)
                ...
                # Remove the clawback hint since it is unnecessary for the LOT coin
        ...
        if fee > 0:
            async with self.new_action_scope(push=False) as inner_action_scope:
                await self.main_wallet.create_tandem_lot_tx(
                    fee,
                    tx_config,
                    inner_action_scope,
                    extra_conditions=(
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        lot_spam_amount = self.config.get("lot_spam_amount", 1000000)

        # No need to filter anything if the filter is set to 1 or 0 mojos
        if lot_spam_amount <= 1:
            return new_coin_state

        spam_filter_after_n_txs = self.config.get("spam_filter_after_n_txs", 200)
        small_unspent_count = await self.coin_store.count_small_unspent(lot_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                    ...
                    and cs.coin.amount < lot_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSLotteryConnection) -> Optional[bytes32]:
        # Get minter DID
    ...
    async def _add_coin_states(
        ...
        self.log.debug("Add coin state target_height: %s",  self.wallet_node.synced_guess_height)
                    ...
                    # if the coin has been spent
                        ...
                        if record is None:
                            ...
                            if len(children) > 0:
                                ...
                                to_puzzle_hash = None
                                memos_dict = None
                                transactionType = TransactionType.OUTGOING_TX.value
                                ...
                                for coin in additions:
                                    ...
                                    if derivation_record is None:  # not change
                                        ...
                                        if coin_spend is None:
                                            ...
                                            memos_dict = compute_memos(SpendBundle([coin_spend], G2Element()))
                                            if coin.puzzle_hash not in GUESS_PUZZLE_HASH:
                                                ...
                                                if stake_metadata is None:
                                                    clawback_metadata = match_clawback_puzzle(
                                                        uncurried, puzzle, solution
                                                    )
                                                else:
                                                    to_puzzle_hash = stake_metadata.recipient_puzzle_hash
                                        if memos_dict is not None and coin.puzzle_hash in GUESS_PUZZLE_HASH:
                                            _name = coin.name()
                                            if _name in memos_dict and len(memos_dict[_name]) > 0:
                                                guess_err, guess = check_guess_memos(
                                                    coin.amount, memos_dict[_name][0]
                                                )
                                                if guess_err is None:
                                                    guess_height = get_guess_height(coin_state.spent_height)
                                                    if self.wallet_node.synced_guess_height >= guess_height:
                                                        guess_num = await self.wallet_node.get_guess_num_by_height(
                                                            guess_height, peer
                                                        )
                                                        guess["B"] = generate_bets(guess_num, guess["v"])
                                                        guess["N"] = guess_num
                                                    if "W" not in guess:
                                                        guess["W"] = coin_state.coin.puzzle_hash.hex()
                                                    memos_dict[_name][0] = json.dumps(guess).encode("utf-8")
                                                    amount = coin.amount
                                                    transactionType = TransactionType.OUTGOING_GUESS_TX.value
                                                    break
                                        elif stake_metadata is not None or clawback_metadata is not None:
                                ...
                                if len(tx_records) > 0:
                                    ...
                                    tx_record = TransactionRecord(
                                        ...
                                        name=tx_name,
                                        memos=list(memos_dict.items()) if memos_dict is not None else [],
                                        valid_times=ConditionValidTimes(),
                                    )
                            ...
                            if record.coin_type == CoinType.CLAWBACK or record.coin_type == CoinType.STAKE:
    ...
    async def reorg_rollback(self, height: int) -> List[uint32]:
        ...
        for record in reorged:
            if TransactionType(record.type) in [
                ...
                TransactionType.OUTGOING_GUESS_TX,
    ...
    async def handle_stake(
        ...
        if await self.puzzle_store.get_derivation_record_for_puzzle_hash(metadata.recipient_puzzle_hash) is not None:
        ...
        if is_recipient is not None:
            ...
            spent_height: uint32 = uint32(0)
            if coin_state.spent_height is not None:
                self.log.debug("Resync stake coin: %s", coin_state.coin.name().hex())
                # Resync case
                spent_height = uint32(coin_state.spent_height)
                # Create Clawback outgoing transaction
                created_timestamp = await self.wallet_node.get_timestamp_for_height(uint32(coin_state.spent_height))
                ...
                if await self.puzzle_store.puzzle_hash_exists(stake_spend_bundle.additions()[0].puzzle_hash):
                    tx_record = TransactionRecord(
                        ...
                        sent_to=[],
                        trade_id=None,
                        type=uint32(TransactionType.STAKE_LOCK_WITHDRAW),
            ...
            await self.coin_store.add_coin_record(coin_record)
            # Add tx record
            # We use TransactionRecord.confirmed to indicate if a Stake transaction is claimable
            ...
            tx_record = TransactionRecord(
                ...
                sent_to=[],
                trade_id=None,
                type=uint32(TransactionType.INCOMING_STAKE_LOCK_RECEIVE),
                # Use coin ID as the TX ID to mapping with the coin table
                name=coin_record.coin.name(),
                memos=list(memos.items()),
                valid_times=ConditionValidTimes(),
        ...
        async with self.new_action_scope(push=True) as action_scope:
            for coin in unspent_coins.records:
                try:
                    ...
                    coin_timestamp = await self.wallet_node.get_timestamp_for_height(coin.confirmed_block_height)
                    if current_timestamp - coin_timestamp >= metadata.time_lock:
                        stake_coins[coin.coin] = metadata
                        if len(stake_coins) >= self.config.get("auto_withdraw_stake", {}).get("batch_size", 50):
                            await self.spend_stake_coins(stake_coins, tx_fee, tx_config, action_scope)
                            async with action_scope.use() as interface:
                                tx_config = dataclasses.replace(
                                    tx_config,
                                    excluded_coin_ids=[
    ...
    async def set_guess_memos(self,  peer: WSLotteryConnection) -> None:
        finished_height = await self.blockchain.get_finished_sync_up_to()
        guess_height = get_guess_height(finished_height, False)
        self.log.info(f"set_guess_memos height: {finished_height} {guess_height} {self.wallet_node.synced_guess_height}")
        if finished_height == 0:
            return
        if self.wallet_node.synced_guess_height+ISSUE_HEIGHT_PER_LOTTERY < guess_height:
            return
        try:
            # start_height = await self.wallet_state_manager.blockchain.get_finished_sync_up_to()
            guess_num = await self.wallet_node.get_guess_num_by_height(guess_height, peer)
            if guess_num is None:
                return
            guess_records = await self.tx_store.get_guess_transactions(
                guess_height - ISSUE_HEIGHT_PER_LOTTERY,
                guess_height,
            )
            if len(guess_records) > 0:
                for record in guess_records:
                    memo = record.memos[0][1][0].decode("utf-8")
                    try:
                        memo_json = json.loads(memo)
                        memo_json["B"] = generate_bets(guess_num, memo_json["v"])
                        memo_json["N"] = guess_num
                        record.memos[0][1][0] = json.dumps(memo_json).encode("utf-8")
                        await self.tx_store.add_transaction_record(record)
                    except json.JSONDecodeError:
                        self.log.error(f"Guess info Invalid JSON string: {memo}")
            await self.blockchain.save_synced_guess_height(guess_height)
        except Exception as e:
            self.log.exception(f"auto_guess_transactions error: {e}")
