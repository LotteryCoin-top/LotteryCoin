...
class WalletBlockchain(BlockchainInterface):
    ...
    synced_guess_height: uint32
    _synced_guess_height: uint32
    ...
    async def create(_basic_store: KeyValStore, constants: ConsensusConstants) -> WalletBlockchain:
        ...
        self.synced_guess_height = await self._basic_store.get_object("SYNCED_GUESS_HEIGHT", uint32)
        if self.synced_guess_height is None:
            self.synced_guess_height = uint32(0)
        self._synced_guess_height = self.synced_guess_height
    ...
    async def set_finished_sync_up_to(self, height: int, *, in_rollback: bool = False) -> None:
        if in_rollback or self.synced_guess_height < self._synced_guess_height:
            await self.save_synced_guess_height(self._synced_guess_height)
    ...
    def set_synced_guess_height(self, height: uint32) -> None:
        if self.synced_guess_height < height:
            self.synced_guess_height = height

    def get_synced_guess_height(self) -> uint32:
        return self.synced_guess_height

    async def save_synced_guess_height(self, height: uint32) -> None:
        self.synced_guess_height = height
        await self._basic_store.set_object("SYNCED_GUESS_HEIGHT", height)
