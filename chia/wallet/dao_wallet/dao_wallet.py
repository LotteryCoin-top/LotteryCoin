...
class DAOWallet:
    ...
    It is not a user-facing wallet. A user cannot spend or receive LOT though a wallet of this type.
    ...
    # if asset_id == None: then we get normal LOT
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSLotteryConnection, coin_data: Optional[Any]) -> None:
        """
        ...
        try:
            ...
            if matched_funding_puz:
                # funding coin
                lot_funds_puzhash = get_p2_singleton_puzhash(self.dao_info.treasury_id, asset_id=None)
                if coin.puzzle_hash == lot_funds_puzhash:
                    asset_id = None
                else:
    ...
    async def resync_treasury_state(self) -> None:
        ...
        wallet_node: Any = self.wallet_state_manager.wallet_node
        peer: WSLotteryConnection = wallet_node.get_full_node_peer()
        if peer is None:  # pragma: no cover
            raise ValueError("Could not find any peers to request puzzle and solution from")
        ...
        # get existing lot funds for treasury
        lot_funds_puzhash = get_p2_singleton_puzhash(self.dao_info.treasury_id, asset_id=None)
        await self.wallet_state_manager.add_interested_puzzle_hashes([lot_funds_puzhash], [self.id()])
    ...
    async def create_proposal_close_spend(
        ...
        if proposal_state["passed"] and not self_destruct:
            ...
            if proposal_type == ProposalType.SPEND:
                ...
                if sum > 0:
                    lot_coins = await self.select_coins_for_asset_type(uint64(sum))
                    for lot_coin in lot_coins:
                        lot_parent_amount_list.append([lot_coin.parent_coin_info, lot_coin.amount])
                        solution = Program.to(
                            [
                                0,
                                treasury_inner_puzhash,
                        ...
                        coin_spends.append(make_spend(lot_coin, p2_singleton_puzzle, solution))
        ...
        if fee > 0:
            await self.standard_wallet.create_tandem_lot_tx(fee, tx_config, action_scope)
            ...
            if prop_info.proposal_id == proposal_id:
                ...
                if proposal_type == ProposalType.SPEND:
                    ...
                    dictionary: Dict[str, Any] = {
                        ...
                        "lot_conditions": lot_created_coins,
