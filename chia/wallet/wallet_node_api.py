...
class WalletNodeAPI:
    ...
    async def respond_removals(self, response: wallet_protocol.RespondRemovals, peer: WSLotteryConnection):
        pass

    async def reject_removals_request(self, response: wallet_protocol.RejectRemovalsRequest, peer: WSLotteryConnection):
        """
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSLotteryConnection):
        """
    ...
    @api_request(peer_required=True)
    async def respond_additions(self, response: wallet_protocol.RespondAdditions, peer: WSLotteryConnection):
        pass
    ...
    @api_request(peer_required=True)
    async def transaction_ack(self, ack: wallet_protocol.TransactionAck, peer: WSLotteryConnection):
        """
    ...
    @api_request(peer_required=True)
    async def respond_peers(self, request: full_node_protocol.RespondPeers, peer: WSLotteryConnection):
    ...
    @api_request(peer_required=True, execute_task=True)
    async def coin_state_update(self, request: wallet_protocol.CoinStateUpdate, peer: WSLotteryConnection):
    ...
    @api_request()
    async def respond_coin_records_by_puzzle_hash(self, request: wallet_protocol.RespondCoinRecords):
        self.log.error("Unexpected message `respond_coin_records_by_puzzle_hash`. Peer might be slow to respond")
        return None
