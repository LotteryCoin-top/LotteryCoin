...
from chia.util.guess import check_guess_memos
...
class Wallet:
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if memos is not None and len(memos) > 0:
            guess_err, _ = check_guess_memos(non_change_amount, memos[0])
    ...
    async def create_tandem_lot_tx(
        ...
        lottery_coins = await self.select_coins(fee, tx_config.coin_selection_config)
    ...
    # WSLotteryConnection is only imported for type checking
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSLotteryConnection, coin_data: Optional[Streamable]
