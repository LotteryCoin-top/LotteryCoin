...
class CATWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSLotteryConnection, parent_coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def create_tandem_lot_tx(
        self,
        fee: uint64,
        ...
        async with self.wallet_state_manager.new_action_scope(push=False) as inner_action_scope:
            if fee > amount_to_claim:
                lottery_coins = await self.standard_wallet.select_coins(
                    fee,
                    tx_config.coin_selection_config,
                )
                origin_id = list(lottery_coins)[0].name()
                ...
                origin_id = list(lottery_coins)[0].name()
                selected_amount = sum(c.amount for c in lottery_coins)
                await self.standard_wallet.generate_signed_transaction(
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some LOT as part of this
        regular_lottery_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_lottery_to_claim = payment_amount

        need_lottery_transaction = (fee > 0 or regular_lottery_to_claim > 0) and (fee - regular_lottery_to_claim != 0)
        ...
        for coin in cat_coins:
            ...
            if first:
                ...
                if need_lottery_transaction:
                    if fee > regular_lottery_to_claim:
                        await self.create_tandem_lot_tx(
                            fee,
                            uint64(regular_lottery_to_claim),
                            tx_config,
                            action_scope,
                    ...
                    elif regular_lottery_to_claim > fee:  # pragma: no cover
                        lot_announcement = await self.create_tandem_lot_tx(
                            fee,
                            uint64(regular_lottery_to_claim),
                            tx_config,
                            action_scope,
                        )
                        assert lot_announcement is not None
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            conditions=(*extra_conditions, lot_announcement, announcement),
                        )
                    else:
