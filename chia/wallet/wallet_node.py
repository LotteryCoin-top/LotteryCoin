...
from chia.util.guess import get_guess_height
...
from chia.wallet.puzzles.stake.metadata import AutoWithdrawStakeSettings
...
class WalletNode:
    ...
    _server: Optional[LotteryServer] = None
    ...
    _guess_header_hashes: Dict[uint32, Optional[List[int]]] = dataclasses.field(default_factory=dict)
    synced_guess_height: uint32 = uint32(0)
    ...
    def get_cache_for_peer(self, peer: WSLotteryConnection) -> PeerRequestCache:
    ...
    async def get_key_for_fingerprint(
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'lottery keys' program.")
            return None
    ...
    async def _process_new_subscriptions(self) -> None:
        while not self._shut_down:
            ...
            peer: Optional[WSLotteryConnection] = None
    ...
    def set_server(self, server: LotteryServer) -> None:
        self._server = server
        self.initialize_wallet_peers()
    ...
    def initialize_wallet_peers(self) -> None:
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.lotterycoin.top"]),
    ...
    async def on_disconnect(self, peer: WSLotteryConnection) -> None:
    ...
    async def on_connect(self, peer: WSLotteryConnection) -> None:
        if self._wallet_state_manager is None:
            return None
    ...
    def is_trusted(self, peer: WSLotteryConnection) -> bool:
        return self.server.is_trusted_peer(peer, self.config.get("trusted_peers", {}))

    async def state_update_received(self, request: CoinStateUpdate, peer: WSLotteryConnection) -> None:
    ...
    def get_full_node_peer(self) -> WSLotteryConnection:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSLotteryConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            raise ValueError("No peer connected")
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSLotteryConnection]:
        """
        ...
        synced_and_trusted: List[WSLotteryConnection] = []
        synced: List[WSLotteryConnection] = []
        trusted: List[WSLotteryConnection] = []
        neither: List[WSLotteryConnection] = []
        all_nodes: List[WSLotteryConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def get_timestamp_for_height_from_peer(self, height: uint32, peer: WSLotteryConnection) -> Optional[uint64]:
        """
    ...
    async def new_peak_wallet(self, new_peak: NewPeakWallet, peer: WSLotteryConnection) -> None:
        if self._wallet_state_manager is None:
        ...
        if latest_timestamp is None or not self.is_timestamp_in_sync(latest_timestamp):
            if trusted:
                ...
                await peer.close(120)
            return
        self.synced_guess_height = get_guess_height(new_peak.height, False)
        if self.is_trusted(peer):
        ...
        async with self.wallet_state_manager.lock:
            ...
            await self.wallet_state_manager.set_guess_memos(peer)
            if self.config.get("auto_withdraw_stake", {}).get("enabled", False):
    ...
    async def new_peak_from_trusted(
        self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSLotteryConnection
    ) -> None:
    ...
    async def new_peak_from_untrusted(self, new_peak_hb: HeaderBlock, peer: WSLotteryConnection) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSLotteryConnection) -> None:
    ...
    async def sync_from_untrusted_close_to_peak(self, new_peak_hb: HeaderBlock, peer: WSLotteryConnection) -> bool:
        async with self.wallet_state_manager.lock:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSLotteryConnection) -> int:
    ...
    async def fetch_and_update_weight_proof(self, peer: WSLotteryConnection, peak: HeaderBlock) -> int:
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSLotteryConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSLotteryConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSLotteryConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    def set_auto_withdraw_stake(self, auto_withdraw_stake_config: AutoWithdrawStakeSettings) -> Dict[str, Any]:
        if auto_withdraw_stake_config.batch_size < 1:
            auto_withdraw_stake_config = dataclasses.replace(auto_withdraw_stake_config, batch_size=uint16(50))
        auto_withdraw_stake_config_json = auto_withdraw_stake_config.to_json_dict()
        if ("auto_withdraw_stake" not in self.config or (
                self.config["auto_withdraw_stake"] != auto_withdraw_stake_config_json)
        ):
            # Update in memory config
            self.config["auto_withdraw_stake"] = auto_withdraw_stake_config_json
            # Update config file
            with lock_and_load_config(self.root_path, "config.yaml") as config:
                ...
                save_config(self.root_path, "config.yaml", config)
        return auto_withdraw_stake_config.to_json_dict()
    ...
    async def get_guess_num_by_height(self, guess_height: uint32, peer: WSLotteryConnection) -> Optional[List[int]]:
        if guess_height in self._guess_header_hashes:
            return self._guess_header_hashes[guess_height]

        request = RequestBlockHeader(guess_height)
        response: Optional[RespondBlockHeader] = await peer.call_api(FullNodeAPI.request_block_header, request)
        if response is None:
            raise ValueError(f"get_header_hash_by_height peer {peer.get_peer_info()} did not respond in time.")
        header_hash = response.header_block.header_hash.hex()
        guess_hash = header_hash[-4:]
        guess_num = []
        for num_str in guess_hash:
            if num_str.isdigit():
                guess_num.append(10 if num_str == "0" else int(num_str))
            else:
                guess_num.append(int(num_str, 16) + 1)
        self._guess_header_hashes[guess_height] = guess_num
        return guess_num
