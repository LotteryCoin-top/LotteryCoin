...
def get_lottery_ca_crt_key() -> Tuple[Any, Any]:
    here = importlib_resources.files(__name__.rpartition(".")[0])
    crt = here.joinpath("lottery_ca.crt").read_bytes()
    key = here.joinpath("lottery_ca.key").read_bytes()
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "LotteryCoin"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "LotteryCoin"),
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("lotterycoin.top")]),
            critical=False,
        )
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "LotteryCoin"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Lottery CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Organic Farming Division"),
        ]
    )
    root_cert = (
...
def create_all_ssl(
    ...
    *,
    ...
    lottery_ca_crt, lottery_ca_key = get_lottery_ca_crt_key()
    lottery_ca_crt_path = ca_dir / "lottery_ca.crt"
    lottery_ca_key_path = ca_dir / "lottery_ca.key"
    write_ssl_cert_and_key(lottery_ca_crt_path, lottery_ca_crt, lottery_ca_key_path, lottery_ca_key, overwrite=overwrite)
...
def main():
    return make_ca_cert(Path("./lottery_ca.crt"), Path("./lottery_ca.key"))
