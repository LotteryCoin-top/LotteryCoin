...
def keys_cmd(ctx: click.Context) -> None:
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with lottery init")
...
@click.option(
    ...
    "-d",
    help=(
        "Show the first wallet address using non-observer derivation. Older Lottery versions use "
...
@click.option("--prefix", "-x", help="Address prefix (lot for mainnet, tlot for testnet)", default=None, type=str)
@click.pass_context
def search_cmd(
    ctx: click.Context,
