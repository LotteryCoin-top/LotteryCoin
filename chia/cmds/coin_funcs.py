    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, config):
        ...
        if not await wallet_client.get_synced():
            print("Wallet not synced. Please wait.")
            return []
        is_lot: bool = wallet_type == WalletType.STANDARD_WALLET  # this lets us know if we are directly combining Lottery
        ...
        if final_target_coin_amount != 0:  # if we have a set target, just use standard coin selection.
            removals: List[Coin] = await wallet_client.select_coins(
                amount=(final_target_coin_amount + fee) if is_lot else final_target_coin_amount,
                wallet_id=wallet_id,
        ...
        if is_lot and total_amount - fee <= 0:
        ...
        additions = [{"amount": (total_amount - fee) if is_lot else total_amount, "puzzle_hash": target_ph}]
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, config):
        ...
        if not await wallet_client.get_synced():
            print("Wallet not synced. Please wait.")
            return []
        is_lot: bool = wallet_type == WalletType.STANDARD_WALLET  # this lets us know if we are directly spitting Lottery
        ...
        if push:
            print(f"Transaction sent: {tx_id}")
            print(f"To get status, use command: lottery wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
        dust_threshold = config.get("lot_spam_amount", 1000000)  # min amount per coin in mojo
