...
def validate_decimal_lot(
    ...
    try:
        ...
        fail_func("Value must be decimal dotted value in LOT (e.g. 0.00005)", param, ctx)
    if d_value.is_signed():
...
class TransactionFeeParamType(click.ParamType):
    """
    A Click parameter type for transaction fees, which can be specified in LOT or mojos.
    """

    name: str = "LOT"  # type name for cli, TODO: Change once the mojo flag is implemented
    ...
    def convert(self, value: Any, param: Optional[click.Parameter], ctx: Optional[click.Context]) -> uint64:
        ...
        d_value = validate_decimal_lot(value, self.fail, param, ctx)
        ...
        try:
            return uint64(d_value * units["lottery"])
        except ValueError:
...
class CliAmount:
    """
    A dataclass for TX / wallet amounts for both LOT and CAT, and of course mojos.
    """
...
class AmountParamType(click.ParamType):
    """
    A Click parameter type for TX / wallet amounts for both LOT and CAT, and of course mojos.
    """
...
class AddressParamType(click.ParamType):
    ...
    def convert(self, value: Any, param: Optional[click.Parameter], ctx: Optional[click.Context]) -> CliAddress:
        ...
        try:
            ...
            if hrp in ["lot", "tlot"]:  # I hate having to load the config here
                addr_type: AddressType = AddressType.LOT
            ...
            else:  # all other address prefixes (Not lot / tlot)
