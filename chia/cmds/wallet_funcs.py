...
transaction_type_descriptions = {
    ...
    TransactionType.OUTGOING_GUESS_TX: "sent guess",
}
...
def print_transaction(
    ...
    if verbose:
        ...
        lottery_amount = Decimal(int(tx.amount)) / mojo_per_unit
        ...
        print(f"Amount {description}: {lottery_amount} {name}")
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if not override and check_unusual_transaction(final_amount, fee):
            print(
                f"A transaction of amount {final_amount / units['lottery']} and fee {fee} is unusual.\n"
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        if offers == [] or requests == []:
            ...
            for item in [*offers, *requests]:
                            ...
                            if item in requests:
                                ...
                                if info.supports_did:
                                    ...
                                    royalty_asset_dict[name] = (
                                        encode_puzzle_hash(info.royalty_puzzle_hash, AddressType.LOT.hrp(config)),
                ...
                if fee > 0:
                    print()
                    print(f"Including Fees: {Decimal(fee) / units['lottery']} LOT, {fee} mojos")
                ...
                if royalty_asset_dict != {}:
                    ...
                    for nft_id, summaries in royalty_summary.items():
                        ...
                        for summary in summaries:
                            divisor = units["lottery"] if summary["asset"] == "LOT" else units["cat"]
                    ...
                    for asset, requested_amount in total_amounts_requested.items():
                        divisor = units["lottery"] if asset == "LOT" else units["cat"]
...
async def print_offer_summary(
    cat_name_resolver: CATNameResolver, sum_dict: Dict[str, int], has_fee: bool = False, network_lot: str = "LOT"
) -> None:
    for asset_id, amount in sum_dict.items():
        description: str = ""
        unit: int = units["lottery"]
        wid: str = "1" if asset_id == "lot" else ""
        mojo_amount: int = int(Decimal(amount))
        name: str = network_lot
        if asset_id != "lot":
            name = asset_id
            if asset_id == "unknown":
                name = "Unknown"
...
async def print_trade_record(record: TradeRecord, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['lottery']} LOT, {fees} mojos")
...
async def take_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        offered, requested, _, _ = offer.summary()
        cat_name_resolver = wallet_client.cat_asset_id_to_name
        network_lot = AddressType.LOT.hrp(config).upper()
        print("Summary:")
        print("  OFFERED:")
        await print_offer_summary(cat_name_resolver, offered, network_lot=network_lot)
        print("  REQUESTED:")
        await print_offer_summary(cat_name_resolver, requested, network_lot=network_lot)
        ...
        for royalty_asset_id in nft_coin_ids_supporting_royalties_from_offer(offer):
            if royalty_asset_id.hex() in offered:
                ...
                royalty_asset_dict[encode_puzzle_hash(royalty_asset_id, AddressType.NFT.hrp(config))] = (
                    encode_puzzle_hash(address, AddressType.LOT.hrp(config)),
                    percentage,
                )
        ...
        if royalty_asset_dict != {}:
            ...
            for fungible_asset_id in fungible_assets_from_offer(offer):
                fungible_asset_id_str = fungible_asset_id.hex() if fungible_asset_id is not None else "lot"
                if fungible_asset_id_str in requested:
                    nft_royalty_currency: str = "Unknown CAT"
                    if fungible_asset_id is None:
                        nft_royalty_currency = network_lot
                    else:
            ...
            if fungible_asset_dict != {}:
                ...
                for nft_id, summaries in royalty_summary.items():
                    print(f"  - For {nft_id}:")
                    for summary in summaries:
                        divisor = units["lottery"] if summary["asset"] == network_lot else units["cat"]
                        converted_amount = Decimal(summary["amount"]) / divisor
                        total_amounts_requested.setdefault(summary["asset"], fungible_asset_dict[summary["asset"]])
                        total_amounts_requested[summary["asset"]] += summary["amount"]
                        print(
                ...
                for asset, amount in total_amounts_requested.items():
                    divisor = units["lottery"] if asset == network_lot else units["cat"]
                    converted_amount = Decimal(amount) / divisor
                    print(f"  - {converted_amount} {asset} ({amount} mojos)")

        print(f"Included Fees: {Decimal(offer.fees()) / units['lottery']} {network_lot}, {offer.fees()} mojos")
        ...
        if not examine_only:
            ...
            res = await wallet_client.take_offer(
                offer,
                fee=fee,
                tx_config=CMDTXConfigLoader().to_tx_config(units["lottery"], config, fingerprint),
                push=push,
            )
            if push:
                print(f"Accepted offer with ID {res.trade_record.trade_id}")
                print(
                    f"Use lottery wallet get_offers --id {res.trade_record.trade_id} -f {fingerprint} to view its status"
                )
...
async def cancel_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        res = await wallet_client.cancel_offer(
            offer_id,
            CMDTXConfigLoader().to_tx_config(units["lottery"], config, fingerprint),
            secure=secure,
            fee=fee,
            push=push,
        )
        ...
        if secure and push:
            print(f"Use lottery wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view cancel status")
...
def wallet_coin_unit(typ: WalletType, address_prefix: str) -> Tuple[str, int]:
    ...
    if typ in [WalletType.STANDARD_WALLET, WalletType.POOLING_WALLET, WalletType.MULTI_SIG]:
        return address_prefix, units["lottery"]
    return "", units["mojo"]
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            response = await wallet_client.create_new_did_wallet(
                amount, CMDTXConfigLoader().to_tx_config(units["lottery"], config, fingerprint), fee, name, push=push
            )
            wallet_id = response["wallet_id"]
...
async def mint_nft(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        royalty_address = royalty_cli_address.validate_address_type(AddressType.LOT) if royalty_cli_address else None
        target_address = target_cli_address.validate_address_type(AddressType.LOT) if target_cli_address else None
        try:
...
async def transfer_nft(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            target_address = target_cli_address.validate_address_type(AddressType.LOT)
...
async def get_nft_royalty_percentage_and_address(
    ...
    info: NFTInfo = NFTInfo.from_json_dict((await wallet_client.get_nft_info(nft_coin_id.hex()))["nft_info"])
    assert info.royalty_puzzle_hash is not None
...
async def send_notification(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        amount: uint64 = cli_amount.convert_amount(units["lottery"])

        tx = await wallet_client.send_notification(address.puzzle_hash, message, amount, fee, push=push)

        if push:
            print("Notification sent successfully.")
            print(f"To get status, use command: lottery wallet get_transaction -f {fingerprint} -tx 0x{tx.name}")
        return [tx]
...
async def sign_message(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        if addr_type == AddressType.LOT:
            if address is None:
                print("Address is required for LOT address type.")
                return
...
async def mint_vc(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        res = await wallet_client.vc_mint(
            ...
            CMDTXConfigLoader().to_tx_config(units["lottery"], config, fingerprint),
            target_address.validate_address_type_get_ph(AddressType.LOT) if target_address else None,
            fee,
            push=push,
        )
