    ...
    with lock_and_load_config(root_path, config_yaml, fill_missing_services=True) as config:
        ...
        if testnet:
            if testnet == "true" or testnet == "t":
                ...
                # check if network_overrides.constants.testnet0 exists
                if (
                    "testnet0" not in config["network_overrides"]["constants"]
                    or "testnet0" not in config["network_overrides"]["config"]
                ):
                    ...
                    if "testnet0" not in config["network_overrides"]["constants"]:
                        config["network_overrides"]["constants"]["testnet0"] = initial_config["network_overrides"][
                            "constants"
                        ]["testnet0"]

                    if "testnet0" not in config["network_overrides"]["config"]:
                        config["network_overrides"]["config"]["testnet0"] = initial_config["network_overrides"][
                            "config"
                        ]["testnet0"]

                testnet_port = "19988"
                testnet_introducer = "introducer-testnet0.lotterycoin.top"
                testnet_dns_introducer = "dns-introducer-testnet0.lotterycoin.top"
                bootstrap_peers = ["testnet0-node-us-west-2.lotterycoin.top"]
                testnet = "testnet0"
                config["full_node"]["peers_file_path"] = "db/peers-testnet0.dat"
                config["wallet"]["wallet_peers_file_path"] = "wallet/db/wallet_peers-testnet0.dat"
                ...
                mainnet_port = "18888"
                mainnet_introducer = "introducer.lotterycoin.top"
                mainnet_dns_introducer = "dns-introducer.lotterycoin.top"
                bootstrap_peers = ["node.lotterycoin.top"]
                net = "mainnet"
        ...
        if change_made:
            print("Restart any running lottery services for changes to take effect")
