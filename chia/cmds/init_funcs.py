...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'lottery keys generate'")
        return None

    with lock_and_load_config(new_root, "config.yaml") as config:
        pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
        all_targets = []
        stop_searching_for_farmer = "lot_target_address" not in config["farmer"]
        stop_searching_for_pool = "lot_target_address" not in config["pool"]
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("lot_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("lot_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("lot_target_address") or all_targets[-2] == config["pool"].get(
                    "lot_target_address"
                ):
        ...
        if "lot_target_address" not in config["farmer"]:
            print(
                f"Setting the lot destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
            config["farmer"]["lot_target_address"] = all_targets[0]
            updated_target = True
        elif config["farmer"]["lot_target_address"] not in all_targets:
            print(
                f"WARNING: using a farmer address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['farmer']['lot_target_address']} with {all_targets[0]}"
            )

        if "pool" not in config:
            config["pool"] = {}
        if "lot_target_address" not in config["pool"]:
            print(f"Setting the lot destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
            config["pool"]["lot_target_address"] = all_targets[0]
            updated_target = True
        elif config["pool"]["lot_target_address"] not in all_targets:
            print(
                f"WARNING: using a pool address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['pool']['lot_target_address']} with {all_targets[0]}"
            )
        if updated_target:
            print(
                f"To change the LOT destination addresses, edit the `lot_target_address` entries in"
...
def init(
    ...
    if create_certs is not None:
        ...
        return lottery_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)
...
def lottery_init(
    ...
    *,
    ...
    """
    lottery_root = os.environ.get("LOTTERY_ROOT", None)
    if lottery_root is not None:
        print(f"LOTTERY_ROOT is set to {lottery_root}")

    print(f"Lottery directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if LOTTERY_ROOT is set, or if user has run lottery init twice
        # before a new update.
        if testnet:
            configure(
                root_path,
    ...
    create_default_lottery_config(root_path)
    if testnet:
        configure(
            root_path,
            set_farmer_peer="",
    ...
    print("")
    print("To see your keys, run 'lottery keys show --show-mnemonic-seed'")
