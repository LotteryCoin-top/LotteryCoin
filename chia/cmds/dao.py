...
@click.option(
    "--proposal-minimum",
    help="The minimum amount (in lot) that a proposal must use to be created",
...
@click.option(
    "--fee-for-cat",
    help="Set the fees for the CAT creation transaction, in LOT.",
...
def dao_create_cmd(
    ...
    return asyncio.run(
        create_dao_wallet(
            ...
            self_destruct,
            oracle_delay,
            proposal_minimum.convert_amount(units["lottery"]),
            filter_amount,
            cat_amount,
            CMDTXConfigLoader(
...
@click.option(
    "--asset-id",
    help="The asset id of the funds the proposal will send. Leave blank for lot",
    type=str,
    required=False,
    default=None,
)
