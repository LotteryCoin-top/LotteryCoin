...
def generate(shell: str) -> None:
    ...
    You can source it directly (performs slower) by running:
        \033[3;33meval "$(lottery complete generate)"\033[0m
    or you can save the output to a file:
        \033[3;33mlottery complete generate > ~/.lottery-complete-bash\033[0m
    and source that file with:
        \033[3;33m. ~/.lottery-complete-bash\033[0m
    """
    ...
    subprocess.run(["lottery"], check=True, env={**os.environ, "_LOTTERY_COMPLETE": f"{shell}_source"})
