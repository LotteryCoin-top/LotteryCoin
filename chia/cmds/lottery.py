...
from chia.cmds.stake import stake_cmd
...
@click.group(
    help=f"\n  Manage lottery blockchain infrastructure ({__version__})\n",
    epilog="Try 'lottery start node', 'lottery netspace -d 192', or 'lottery show -s'",
...
@cli.command("version", help="Show lottery version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", help="Runs lottery daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by lottery start <service>
)
@click.pass_context
