...
class SyncLotteryCommand(Protocol):
    def run(self) -> None: ...


class AsyncLotteryCommand(Protocol):
    async def run(self) -> None: ...


LotteryCommand = Union[SyncLotteryCommand, AsyncLotteryCommand]
...
class _CommandParsingStage:
    my_dataclass: Type[LotteryCommand]
    ...
    def initialize_instance(self, **kwargs: Any) -> LotteryCommand:
...
def _generate_command_parser(cls: Type[LotteryCommand]) -> _CommandParsingStage:
...
def _convert_class_to_function(cls: Type[LotteryCommand]) -> SyncCmd:
...
@dataclass_transform()
def lottery_command(cmd: click.Group, name: str, help: str) -> Callable[[Type[LotteryCommand]], Type[LotteryCommand]]:
    def _lottery_command(cls: Type[LotteryCommand]) -> Type[LotteryCommand]:
        ...
        if sys.version_info < (3, 10):  # pragma: no cover
            ...
            wrapped_cls: Type[LotteryCommand] = dataclass(  # type: ignore[assignment]
                frozen=True,
            )(cls)
        else:
            wrapped_cls: Type[LotteryCommand] = dataclass(  # type: ignore[assignment]
