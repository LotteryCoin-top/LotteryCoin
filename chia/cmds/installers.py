...
def check_plotter(plotter: List[str], expected_output: bytes, specify_tmp: bool = True) -> None:
    with tempfile.TemporaryDirectory() as path:
        ...
        process = subprocess.Popen(
            [executable_for_service("lottery"), "plotters", *plotter, *tmp_dir, "--final_dir", path],
...
@click.option("--expected-lottery-version", "expected_lottery_version_str", required=True)
@click.option("--require-madmax/--require-no-madmax", "require_madmax", default=True)
def test_command(expected_lottery_version_str: str, require_madmax: bool) -> None:
    print("testing installed executables")
    expected_lottery_version = packaging.version.Version(expected_lottery_version_str)

    args = [executable_for_service("lottery"), "version"]
    print(f"launching: {args}")
    lottery_version_process = subprocess.run(
        args,
        capture_output=True,
        encoding="utf-8",
        timeout=adjusted_timeout(30),
    )
    assert lottery_version_process.returncode == 0
    assert lottery_version_process.stderr == ""

    lottery_version = packaging.version.Version(lottery_version_process.stdout)
    print(lottery_version)
    assert lottery_version == expected_lottery_version, f"{lottery_version} != {expected_lottery_version}"

    args = [executable_for_service("lottery"), "plotters", "version"]
    print(f"launching: {args}")
    ...
    # check_plotter(plotter=["madmax"], expected_output=b"Multi-threaded pipelined Lottery")
    # check_plotter(plotter=["bladebit", "diskplot", "--compress", "0"], expected_output=b"\nBladebit Lottery Plotter")
    ...
    args = [executable_for_service("lottery"), "init"]
    print(f"launching: {args}")
    subprocess.run(
        args,
        check=True,
