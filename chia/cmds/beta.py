...
from chia.cmds.beta_funcs import (
    ...
    prepare_lottery_blockchain_log,
    prepare_logs,
...
def print_restart_warning() -> None:
    print("\nRestart the daemon and any running lottery services for changes to take effect.")
...
def configure(ctx: click.Context, path: Optional[str], interval: Optional[int]) -> None:
    ...
    with lock_and_load_config(root_path, "config.yaml") as config:
        if "beta" not in config:
            raise click.ClickException("beta test mode is not enabled, enable it first with `lottery beta enable`")
...
def prepare_submission_cmd(ctx: click.Context) -> None:
    with lock_and_load_config(ctx.obj["root_path"], "config.yaml") as config:
        ...
        if beta_root_path is None:
            raise click.ClickException("beta test mode not enabled. Run `lottery beta enable` first.")
    ...
    lottery_blockchain_path = Path(prepare_result / "LotteryCoin")
    lottery_logs = prepare_logs(plotting_path, prepare_lottery_blockchain_log)
    plotting_logs = prepare_logs(lottery_blockchain_path, prepare_plotting_log)
    ...
    with zipfile.ZipFile(submission_file_path, "w", zipfile.ZIP_DEFLATED) as zip_file:
        files_added = add_files(lottery_logs) + add_files(plotting_logs)

    if files_added == 0:
        submission_file_path.unlink()
        message = f"No logs files found in {str(plotting_path)!r} and {str(lottery_blockchain_path)!r}."
